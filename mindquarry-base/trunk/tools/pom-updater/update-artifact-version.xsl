...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://maven.apache.org/POM/4.0.0" xmlns:pom="http://maven.apache.org/POM/4.0.0" exclude-result-prefixes="pom">
	...
	<xsl:template match="pom:version">
		<xsl:choose>
			<xsl:when test="(starts-with(../pom:artifactId,$artifactId)) and ((starts-with(../pom:groupId,$groupId)) or (not(../pom:groupId) and (../pom:parent/pom:groupId=$groupId)))">
				<!-- Silbling groupId and artifactId match (if sibling groupId is missing then the groupId of the parent POM must match) -->
				<xsl:message>Replaced version '<xsl:value-of select="."/>' with '<xsl:value-of select="concat(substring-before(., '-SNAPSHOT'), $version)"/>' for artifact '<xsl:value-of select="$groupId"/>:<xsl:value-of select="../pom:artifactId"/>' in file '<xsl:value-of select="$file"/>'.</xsl:message>
				<version><xsl:value-of select="concat(substring-before(., '-SNAPSHOT'), $version)"/></version>
			</xsl:when>
			<xsl:otherwise>
