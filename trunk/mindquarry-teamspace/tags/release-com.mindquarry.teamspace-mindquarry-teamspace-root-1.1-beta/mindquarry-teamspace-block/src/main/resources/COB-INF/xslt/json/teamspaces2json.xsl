...
<xsl:template match="/teamspaces">
{
  ...
  "response":{
    ...
    "docs":{
      "teams":[
        <xsl:apply-templates select="teamspace[contains(.,$query)]"/>
      ]
    }
  }
}
...
<xsl:template match="teamspace"> {"location":"<xsl:value-of select="$basePath" />team/<xsl:value-of select="id" />/", "title": "<xsl:call-template name="strip-quotes">
...
</xsl:call-template>", "picture":"<xsl:value-of select="$basePath" /><xsl:value-of select="id" />.22.png"}
