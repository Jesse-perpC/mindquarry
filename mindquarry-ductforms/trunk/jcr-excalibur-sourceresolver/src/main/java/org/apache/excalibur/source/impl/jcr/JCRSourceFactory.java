...
public class JCRSourceFactory implements ThreadSafe, SourceFactory, Configurable, Serviceable {
     ...
     * allows queries that return one node with no children (because JCRNodeSource
     * is a simple, non-xmlizable Source, that does not know what to do if someone
     ...
     */
    protected Source executeQuery(Session session, String statement, String queryLang) throws IOException {
    	if (queryLang == Query.SQL &&
    			!session.getRepository().getDescriptor(Repository.OPTION_QUERY_SQL_SUPPORTED).equals("true")) {
    		throw new SourceException("JCR Repository does not support SQL queries.");
    	}
        ...
        try {
			...
			return new JCRNodeSource(this, nodeIter.nextNode());			
		} catch (RepositoryException e) {
            throw new SourceException("Cannot execute query '" + statement + "'", e);
		}
	}
